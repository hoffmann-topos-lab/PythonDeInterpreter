"""
Script de debug para o pipeline MicroPython.

Análogo a Decompiler/debug_stages.py, mas para arquivos .mpy.

Uso:
    cd v0.5/
    python3.12 MicroPython/mpy_debug_stages.py <arquivo.mpy> --stage <stage> [--debug]

Stages disponíveis:
    dis          — exibe bytes do bytecode (hex dump)
    parse        — instruções decodificadas (mpy_disasm)
    blocks       — blocos básicos
    cfg          — grafo de fluxo de controle
    stack        — simulação de pilha (IN/OUT + stmts + conds por bloco)
    native_asm   — disassembly de código nativo (funções @micropython.native/viper)
    gen_code     — código Python recuperado (pipeline completo)
"""
...
# Garante que v0.5/ está no path (independente de onde é chamado)
